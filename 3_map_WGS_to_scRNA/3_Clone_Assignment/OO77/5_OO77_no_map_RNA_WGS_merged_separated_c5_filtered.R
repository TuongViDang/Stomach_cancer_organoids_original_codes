...
plot_dir = file.path(main_dir,"plots","map_no_merged_separated_filtered")
...
#Reload
Combined_all = read.delim(file.path(txt_files,paste0(patient,"_Combined_all_merged_separated_c5.txt")))
dim(Combined_all)
...
write.table(Combined_all, file.path(txt_files,paste0(patient,"_Combined_all_merged_separated_c5_filtered.txt")), sep = "\t", quote = F)
...
#save this seurat
saveRDS(seurat_sub, file.path(rds_dir,paste0(organoid,"_Clones_merged_separated_c5_filtered.RDS")))

#load the file
seurat_sub = readRDS(file.path(rds_dir,paste0(organoid,"_Clones_merged_separated_c5_filtered.RDS")))
...
ggsave(file.path(plot_dir_sub,paste0(patient,"_Untreated_sub_by_Clone_all_cell.pdf")),width = 10000, height= 8000 ,dpi = 1500,units = 'px')
