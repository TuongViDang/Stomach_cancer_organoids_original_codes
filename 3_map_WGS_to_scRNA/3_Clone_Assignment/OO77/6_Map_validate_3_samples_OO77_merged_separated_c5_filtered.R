...
plot_dir = file.path(Integrate_dir,"plots","Map_validate_3_samples_merged_separated_c5_filtered")
...
Combined_all = read.table( file.path(txt_files,paste0(patient,"_Combined_all_merged_separated_c5_filtered.txt")))

Combined_all = left_join(Combined_all, annotation[, c("mutation_id","Func_Annovar")], by = "mutation_id")
...
seurat_no = readRDS(file.path(rds_dir, paste0(patient,"-no_Clones_merged_separated_c5_filtered.RDS" )))


seurat_vitro = readRDS(file.path(rds_dir, paste0(patient,"-vitro_Clones_merged_separated_c5_filtered.RDS")))


seurat_vivo =  readRDS(file.path(rds_dir, paste0(patient,"-vivo_Clones_merged_separated_c5_filtered.RDS" )))
...
plot <- DimPlot(seurat_patient, group.by= "treatment", cols = colors, raster = T, pt.size = 1.5) + labs(title = patient)
plot 
ggsave(file.path(plot_dir,paste0(patient,"_3_samples_sub_DimPlot_all_cell.pdf")),width = 1, height= 8)
...
#Plot the mutation ALT and REF of PIK3CA

Plot_mutation_cells <- function(mutation_id){
mut = mutation_id
...
ggsave(paste0(patient,"_",gene,".pdf"),path = plot_dir,width = 6,height = 5)

write.table(plot_data, file.path(Figures_data, paste0("Figure_4D_scRNA_detected_mut_",gene,".txt")), sep = "\t", quote = F)
}

Combined_all %>% filter(Gene_name == "PIK3CA") %>% distinct(mutation_id, .keep_all = T) 
Plot_mutation_cells(":3:179234297:A:G")


Combined_all %>% filter(Gene_name == "CDH1") %>% distinct(mutation_id, .keep_all = T) 

Plot_mutation_cells(":16:68822117:C:T")
...
rows_gap = c(487, 487 + 370, 487 + 370 + 266)
table(anno_cell$Clone)
cols_gap = c(2313 , 2313 + 2488, 2313 + 2488 +  936 )
...
#Save Figures data
write.table(mutation_matrix, file.path(Figures_data, paste0( "Figure_Sup_6A_mutation_matrix_", patient,".txt")), sep = "\t", quote= F)
write.table(anno_mut,  file.path(Figures_data, paste0( "Figure_Sup_6A_mutation_matrix_annotation_mutation_", patient,".txt")), sep = "\t", quote= F)
write.table(anno_cell,  file.path(Figures_data, paste0("Figure_Sup_6A_mutation_matrix_annotation_cell_", patient, ".txt")), sep = "\t", quote= F)
...
#Save data
write.table(mut_scale_mat, file.path(Figures_data, paste0( "Figure_Sup_6A_expression_matrix_", patient,".txt")), sep = "\t", quote= F)
...
pheatmap(mutation_matrix_c[,1:333], color = c("whitesmoke","red"),
        cluster_rows = F, cluster_cols = F,
        show_rownames = F, show_colnames = F,
        annotation_row = anno_mut ,
        annotation_col = head(anno_cell_sample, 333), annotation_colors = anno_colors ,
        legend_breaks = c(0, 1),
        filename = file.path(plot_dir,"mutation_clone_2_333cells.pdf"), width = 6.5 ,height = 7)


heatmap_img <- image_read(file.path(plot_dir, "mutation_clone_2_333cells.png"), density = 600)
# Apply Gaussian Blur
blurred_img <- image_blur(heatmap_img, radius = 10, sigma = 1.5)

# Save the blurred image
image_write(blurred_img, path = file.path(plot_dir, "mutation_clone_2_333cells_blurred.pdf"),  density = 600,format = "pdf")

#mark mutations
muts = ":12:22065183:G:A"
...
pheatmap(mutation_matrix_c[,1:334], color = c("whitesmoke","red"),
        ...
        annotation_col = head(anno_cell_sample, 333), annotation_colors = anno_colors ,
        legend_breaks = c(0, 1),
        filename = file.path(plot_dir,"mutation_clone_2_333cells_rownames.png"), width = 10 ,height= 8)
...
#save Figure data
write.table(Purity, file.path(Figures_data, "Figure_Sup_6A_Purity_cell_clone_OO77.txt"), sep = "\t", quote = F)
...
Clone_sample <- read.delim(file.path(txt_files, "Clone_proportion.txt"))
...
# Plot proportions as percentages
Clone_sample %>%
  ggplot(aes(x = factor(sample, levels = rev(ordered_samples)), y = proportion, fill = factor(Clone, levels = rev(ordered_clones)))) +
...
# Save the plot
ggsave(file.path(plot_dir, "OO77_clone_proportion.pdf"), width = 13, height = 6)
...
plot
# Save the plot
ggsave(file.path(plot_dir, "OO77_clone_proportion_vertical.pdf"), width = 8 , height = 8, scale = 1)
...
ggsave(file.path(plot_dir, paste0(patient,"_fraction_cell_in_mutation_clone.pdf")), width = 11, height = 13.5, scale=1)
...
Contaminate_mut_each_clone = purrr::map_dfr(clones_list_al2 ,~Contaminate_mut_fraction(.x, clones_cell_list = clones_list_al2 , mutation_matrix = mutation_matrix, anno_mut = anno_mut, anno_cell=anno_cell))
