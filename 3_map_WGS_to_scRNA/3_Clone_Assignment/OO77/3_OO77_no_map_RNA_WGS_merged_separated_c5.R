...
plot_dir = file.path(main_dir,"plots","map_no_merged_separated")
...
#Modify treeCLUSTER based on new cluster 5
new_c5 = read.delim(file.path(txt_files,"OO77_Cluster_5_merged_separation.txt"))
mut_5a = new_c5  %>% filter(cluster == "5a") %>% pull(mutation_id)
mut_5b = new_c5  %>% filter(cluster == "5b") %>% pull(mutation_id)

Combined_all = Combined_all  %>% mutate(treeCLUSTER_merged = case_when(mutation_id %in% mut_5a ~ "5a",
                                                                       mutation_id %in% mut_5b ~ "5b",
                                                                       TRUE ~ as.character(treeCLUSTER )) )
Combined_all$treeCLUSTER = Combined_all$treeCLUSTER_merged
write.table(Combined_all, file.path(txt_files,paste0(patient,"_Combined_all_merged_separated_c5.txt")), sep = "\t", quote = F)
...
#save this seurat
saveRDS(seurat_sub, file.path(rds_dir,paste0(organoid,"_Clones_merged_separated_c5.RDS")))

#load the file
seurat_sub = readRDS(file.path(rds_dir,paste0(organoid,"_Clones_merged_separated_c5.RDS")))