...
#save seurat
saveRDS(seurat_patient, file.path(rds_dir,paste0(patient,"_three_samples_final_Clones.rds")))
...
ggplot(plot_data, aes(x = umap_1, y = umap_2, color = Clone)) +
  ...
  geom_point_rast(data = subset(plot_data, Clone == "UnI"), size = 0.5) +
...
rows_gap = c( 2 + 33 +103 ,2 + 33 +103 +  8 +  5 + 16 +  3 +  3, 2 + 33+ 103 +  8 +  5 + 16 +  3 +  3 + 28 )
table(anno_cell$Clone)
cols_gap = c(  4 + 117 + 318 , 4 + 117 + 318 +56 +  8+  43 +  6 + 35, 4 + 117 + 318 +56 +  8+  43 +  6 + 35 + 92  )
...
pheatmap(mutation_matrix, color = c("whitesmoke","red"),
        ...
        legend_breaks = c(0, 1),
        filename = file.path(plot_dir,"mutation_matrix_3_samples_heatmap.pdf"), width = 6,height=7)

#Save Figures data
write.table(mutation_matrix, file.path(Figures_data, paste0( "Figure_Sup_6B_mutation_matrix_", patient,".txt")), sep = "\t", quote= F)
write.table(anno_mut,  file.path(Figures_data, paste0( "Figure_Sup_6B_mutation_matrix_annotation_mutation_", patient,".txt")), sep = "\t", quote= F)
write.table(anno_cell,  file.path(Figures_data, paste0("Figure_Sup_6B_mutation_matrix_annotation_cell_", patient, ".txt")), sep = "\t", quote= F)
...
#Save data
write.table(mut_scale_mat, file.path(Figures_data, paste0( "Figure_Sup_6B_expression_matrix_", patient,".txt")), sep = "\t", quote= F)
...
pheatmap(mutation_matrix_c, color = c("whitesmoke","red"),
        ...
        legend_breaks = c(0, 1),
        filename = file.path(plot_dir,"mutation_clone_4.png"), width = 6.5 ,height = 7)
...
#save Figure data
write.table(Purity, file.path(Figures_data, "Figure_Sup_6B_Purity_cell_clone_OO99.txt"), sep = "\t", quote = F)
...
# Save the plot
plot
ggsave(file.path(plot_dir, "OO99_clone_proportion_vertical.pdf"), width = 8 , height = 8, scale = 1)
