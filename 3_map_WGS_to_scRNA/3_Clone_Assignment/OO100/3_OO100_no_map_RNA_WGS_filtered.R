...
plot_dir = file.path(main_dir,"plots","map_no_filtered")
...
#Change the name of clone
 ...
 Combined_all <- Combined_all %>% mutate(treeCLUSTER_new = case_when(treeCLUSTER == 14 ~ 2,
                                                                     treeCLUSTER == 5 ~ 3,
                                                                     treeCLUSTER == 7 ~ 4,
                                                                     treeCLUSTER == 23 ~ 5,
                                                                     treeCLUSTER == 4 ~ 6,
...
table(Combined_all$treeCLUSTER)
#remove the mutation in the fiter of co_occur
Mut_remove = read.delim(file.path(txt_files, paste0(patient,"mut_co_occur_remove.txt")))

Combined_all = Combined_all %>% filter(!(mutation_id %in% Mut_remove$mutation_id))
dim(Combined_all)


write.table(Combined_all, file.path(txt_files,paste0(patient,"_Combined_all_filtered.txt")), sep = "\t", quote = F)
...
clone_columns = c( "2", "3","4","5", "7" ,"8","9","10","11","12")
...
mutated_CB <- mutated_CB %>% 
  mutate(Clone_pri = case_when(
    #Clone %in% c("2","3","4") & `5` > 0 ~ "5",
    Clone %in% c("2","3") & `4` > 0 ~ "4",
    Clone == "2" & `3` > 0 ~ "3",
    Clone %in% c("7","8") & `9` > 0 ~ "9",
    Clone == "7" & `8` > 0 ~ "8",
    Clone %in% c("10","11") & `12` > 0 ~ "12",
    Clone == "10" & `11` > 0 ~ "11",
    TRUE ~ Clone
  ))
...
#save this seurat
saveRDS(seurat_sub, file.path(rds_dir,paste0(organoid,"_Clones_filtered.RDS")))

#load the file
seurat_sub = readRDS(file.path(rds_dir,paste0(organoid,"_Clones_filtered.RDS")))
# Add "UnI" to the levels of the factor
seurat_sub$Clone <- factor(seurat_sub$Clone, levels = c(levels(seurat_sub$Clone), "UnI"))
# Replace NA values with "UnI"
seurat_sub$Clone[is.na(seurat_sub$Clone)] <- "UnI"
table(seurat_sub$Clone)
...
p <-  DimPlot(seurat_sub, reduction = "umap", group.by = "Clone") + theme_minimal()
plot_data <- p$data
# Convert `Clone` to factor and ensure `NA` is handled properly
plot_data$Clone <- factor(plot_data$Clone, levels =  names(clone_colors))
...
ggplot(plot_data, aes(x = umap_1, y = umap_2, color = Clone)) +
  ...
  geom_point(data = subset(plot_data, Clone == "7"), size = 0.8) +
  geom_point(data = subset(plot_data, Clone == "8"), size = 0.8) +
  geom_point(data = subset(plot_data, Clone == "9"), size = 0.8) +
  geom_point(data = subset(plot_data, Clone == "10"), size = 0.8) +
  geom_point(data = subset(plot_data, Clone == "11"), size = 0.8) +
  geom_point(data = subset(plot_data, Clone == "12"), size = 0.8) +
  geom_point(data = subset(plot_data, Clone == "2"), size = 0.8) +
  geom_point(data = subset(plot_data, Clone == "3"), size = 0.8) +
  geom_point(data = subset(plot_data, Clone == "4"), size = 0.8) +
   geom_point(data = subset(plot_data, Clone == "5"), size = 0.8) +
...
ggsave(file.path(plot_dir_sub,paste0(patient,"_Untreated_by_Clone_all_cell.pdf")),width = 10000, height= 8000 ,dpi = 1500,units = 'px')
