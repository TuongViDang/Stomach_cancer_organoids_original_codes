...
#SBATCH -J OO99_prepare  # name of the job   
#SBATCH -D /group/poetsch_projects/poetsch_sc/Conifer/log    # folder for job error+logs.
#SBATCH -o OO99_prepare.log               # job log name
#SBATCH -e OO99_prepare.err               # job error output name (error output just means output without log flag - not all of it is errors)
 


patient=OO99
Mutect_dir=/group/poetsch_projects/poetsch_sc/scrna_gastric_cancer_230613/CANOPY2/$patient/Somatic/MuTect2
Mutect_file=$Mutect_dir/OO99.filtered.PASS.vcf
...
grep -v "^#" $Mutect_file | awk -F'\t' '{split($10, LF, ":"); split(LF[2],AD,","); print "OO99""\t""OO99_ex""\t"$1"\t"$2"\t"$4"\t"$5"\t"AD[1]"\t"AD[2]"\t"LF[4]}' \
  > $Input_dir/OO99_ex.mutect.tsv

grep -v "^#" $Mutect_file | awk -F'\t' '{split($11, LF, ":"); split(LF[2],AD,","); print "OO99""\t""OO99_in""\t"$1"\t"$2"\t"$4"\t"$5"\t"AD[1]"\t"AD[2]"\t"LF[4]}' \
  > $Input_dir/OO99_in.mutect.tsv

grep -v "^#" $Mutect_file | awk -F'\t' '{split($13, LF, ":"); split(LF[2],AD,","); print "OO99""\t""OO99_tumor""\t"$1"\t"$2"\t"$4"\t"$5"\t"AD[1]"\t"AD[2]"\t"LF[4]}' \
  > $Input_dir/OO99_tumor.mutect.tsv

grep -v "^#" $Mutect_file | awk -F'\t' '{split($14, LF, ":"); split(LF[2],AD,","); print "OO99""\t""OO99_tumor2""\t"$1"\t"$2"\t"$4"\t"$5"\t"AD[1]"\t"AD[2]"\t"LF[4]}' \
  > $Input_dir/OO99_tumor2.mutect.tsv


samples=(OO99_ex OO99_in OO99_tumor OO99_tumor2)
