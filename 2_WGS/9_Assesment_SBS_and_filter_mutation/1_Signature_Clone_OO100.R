...
samples_order = c( "Clone_1" ,"Clone_14", "Clone_5" ,"Clone_7", "Clone_23","Clone_4","Clone_18","Clone_2", "Clone_19","Clone_22","Clone_6","Clone_11")
SBS_long$Samples <- factor(SBS_long$Samples, levels= samples_order )
table(SBS_long$Signature)
SBS_long$Signature <- factor(SBS_long$Signature, levels= rev(c(   "SBS1",       "SBS5",     "SBS2",   "SBS10a",  "SBS18",  "SBS36", "SBS40a",    "SBS40b",      "SBS41",    "SBS89",   "SBS93",   "SBS96",  "SBS51","SBS53","SBS57","SBS58")))

cols_SBS = rev(c("cadetblue3","cadetblue2","slateblue1","burlywood3","hotpink1","plum","mistyrose1","cyan2", "cornsilk1",   "lavenderblush1",  "khaki2",  "wheat2",  rep("gray",4)))
...
ggsave(file.path(plot_dir,"SBS_signature.pdf"), width = 8,height = 5 )
...
SBS_long %>% ggplot(aes(x= Samples,y=Count,fill=Signature))+ geom_bar(stat="identity",position="fill",width=0.6) + scale_fill_manual(values=cols_SBS) + theme_void() + 
 ylab("Number of mutations in each signature") + guides(fill = guide_legend(reverse = TRUE))+
   theme(legend.title = element_blank(),
        ...
        axis.title.x.bottom = element_blank())   
ggsave(file.path(plot_dir,"SBS_signature_relative.pdf"), width = 8,height = 5)
...
#remove mutations has probability of SBS artifact >= 0.5
