...
patient = "OO99"
tree_df = read.delim(file.path("/group/poetsch_projects/poetsch_sc/Conifer",patient,"output/tree_30/Trees/allTrees.txt"),comment.char = "#",header=F)
...
Clones_all = read.delim(file.path("/group/poetsch_projects/poetsch_sc/Conifer",patient,"output/tree_30/Trees/cloneProportionsMinErrorTrees.txt"))
Clones_all <- Clones_all %>% mutate(OO99_tumor_both  = (OO99_tumor + OO99_tumor2)/2)
...
tumor_new = purrr::map(Clones_all$clusterID,~ New_proportion(.x,sample = paste0(patient,"_tumor_both"),clones_df = Clones_all, tree=tree_df)) %>% unlist()
...
Observed_CCF_clone_proportion_plots <- function(Clones_all_new, Clones_all, value_column, sample_name, plot_dir, patient, colors ) {
  ...
  mutate(
    ...
    label = ifelse(percent > 0.10, sprintf("%.0f", percent * 100), NA),
    pos = cumsum(Proportion) - 0.5 * Proportion
  )

  p2 <- ggplot(label_data, aes(x = Sample, y = Proportion, fill = clusterID)) +
  geom_bar(stat = "identity", position = "stack", width = 0.4) +
  geom_text(aes(y = pos, label = label), color = "black",hjust = 1.9, size = 11) +
  ...
  # Combine plots
  combined_plot <- p1 + p2 + plot_layout(widths = c(5.3, 2.2)) +
    plot_annotation(title = "Clone proportion (%)",theme = theme(plot.title = element_text(size = 32)))
  
  ggsave(file.path(plot_dir, paste0(patient, "_Observed_CCF_Clone_proportion_", value_column, ".pdf")), plot = combined_plot, width = 7.2, height = 4,  scale = 1)
...
#Save data
write.table(Clones_all_new,file.path(Figures_data, paste0("Figure_3B_bulkWGS_Clone_evolution_",patient,".txt")), sep = "\t", quote = F)
...
ggsave(file.path(plot_dir, paste0(patient, "_Recent_expanding_clone.pdf")), width = 4, height = 4)
