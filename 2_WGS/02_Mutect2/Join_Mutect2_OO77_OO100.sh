...
mkdir -p $align_dir
cd $align_dir
samples=(${patient}_ex ${patient}_in ${patient}_tumor ${patient}_normal)
...
gatk Mutect2 -R $REF -I $align_dir/$tumor.filter.recal.bam  -I $align_dir/$ex.filter.recal.bam -I $align_dir/$in.filter.recal.bam -I $align_dir/$normal.filter.recal.bam -normal ${patient}_${patient}_normal -O $MuTect2_dir/${patient}.somatic.vcf.gz
