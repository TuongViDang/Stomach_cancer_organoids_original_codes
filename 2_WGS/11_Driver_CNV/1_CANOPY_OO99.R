...
tumor2 = read.delim(file.path(Ascat_dir, tumor2, "result",paste0(patient,"_",tumor2,".copynumber.caveman.txt")), header=F)
colnames(tumor2) <- c("CHR","START","END","M","m")
tumor2$M <- tumor2$M - tumor2$m

tumor2_gr <- GRanges(
    seqnames = tumor2$CHR,
    ranges = IRanges(tumor2$START, end = tumor2$END),
    sample = rep("tumor2",nrow(tumor2)),
    M = tumor2$M,
    m= tumor2$m)
...
all_gr = c(tumor_gr,tumor2_gr,ex_gr,inv_gr)
...
tumor2_new = purrr::map_dfr(1:nrow(disjoint_df),~CN_for_disjoint_seq(.x,df=tumor2))
tumor2_new = tumor2_new %>% mutate( cnv_id = paste0(sub("chr","",chr) ,":",  start  , ":",  end ), sample = paste0(patient,"_tumor2"))
tumor2_new_M = tumor2_new %>% select(cnv_id,sample, M)
tumor2_new_m = tumor2_new %>% select(cnv_id,sample, m)
...
M_data <- rbind(tumor_new_M, tumor2_new_M,  ex_new_M, inv_new_M)
...
columns_to_select = c(paste0("M_value_", patient, "_tumor"),
                      paste0("M_value_", patient, "_tumor2"),
...
drivers_cn = as.data.frame(drivers_cn)
...
classify_cnv <- function(M_data, m_data, patient) {
  ...
  identical_pair <- function(values1, values2) {
    return(identical_all(values1) && identical_all(values2))
  }

  identical_any_three <- function(values) {
    combn(values, 3, FUN = function(x) identical_all(x), simplify = TRUE) %>% any()
  }
  
  # Create dynamic column names based on patient_id
  M_tumor_col <- paste0("M_value_", patient, "_tumor")
  M_tumor2_col <- paste0("M_value_", patient, "_tumor2")
  ...
  combined_data <- combined_data %>%
    ...
    mutate(group = case_when(
      # Step 1: Check if all values are identical across the four columns
      identical_all(c_across(all_of(c(M_tumor_col, M_tumor2_col, M_ex_col, M_in_col)))) &
      identical_all(c_across(all_of(c(m_tumor_col, m_tumor2_col, m_ex_col, m_in_col)))) ~ "group 1",
      
      # Check if any three values are identical (four combinations)
      identical_any_three(c_across(all_of(c(M_tumor_col, M_tumor2_col, M_ex_col)))) &
      identical_any_three(c_across(all_of(c(m_tumor_col, m_tumor2_col, m_ex_col)))) ~ "group 2",
      identical_any_three(c_across(all_of(c(M_tumor_col, M_tumor2_col, M_in_col)))) &
      identical_any_three(c_across(all_of(c(m_tumor_col, m_tumor2_col, m_in_col)))) ~ "group 3",
      identical_any_three(c_across(all_of(c(M_tumor_col, M_ex_col, M_in_col)))) &
      identical_any_three(c_across(all_of(c(m_tumor_col, m_ex_col, m_in_col)))) ~ "group 4",
      identical_any_three(c_across(all_of(c(M_tumor2_col, M_ex_col, M_in_col)))) &
      identical_any_three(c_across(all_of(c(m_tumor2_col, m_ex_col, m_in_col)))) ~ "group 5",

      # Check pairwise similarities
      identical_pair(c_across(all_of(c(M_tumor_col, M_tumor2_col))), c_across(all_of(c(m_tumor_col, m_tumor2_col)))) ~ "group 6",
      identical_pair(c_across(all_of(c(M_tumor_col, M_ex_col))), c_across(all_of(c(m_tumor_col, m_ex_col)))) ~ "group 7",
      identical_pair(c_across(all_of(c(M_tumor_col, M_in_col))), c_across(all_of(c(m_tumor_col, m_in_col)))) ~ "group 8",
      identical_pair(c_across(all_of(c(M_tumor2_col, M_ex_col))), c_across(all_of(c(m_tumor2_col, m_ex_col)))) ~ "group 9",
      identical_pair(c_across(all_of(c(M_tumor2_col, M_in_col))), c_across(all_of(c(m_tumor2_col, m_in_col)))) ~ "group 10",
      identical_pair(c_across(all_of(c(M_ex_col, M_in_col))), c_across(all_of(c(m_ex_col, m_in_col)))) ~ "group 11",

      TRUE ~ "group X"
    )) %>%
    rowwise() %>%
    mutate(group_2 = case_when(
      # Step 2: If group is "group X", check further for specific conditions
      (group == "group X") &
      ((identical_all(c_across(all_of(c(m_tumor_col, m_tumor2_col, m_ex_col, m_in_col))))) | 
      (identical_all(c_across(all_of(c(M_tumor_col, M_tumor2_col, M_ex_col, M_in_col))))))  ~ "group 12",

      (group == "group X") &
      ((identical_all(c_across(all_of(c(m_tumor_col, m_tumor2_col))))) |
      (identical_all(c_across(all_of(c(M_tumor_col, M_tumor2_col)))))) ~ "group 13",

      (group == "group X") &
      ((identical_all(c_across(all_of(c(m_tumor_col, m_ex_col))))) |
      (identical_all(c_across(all_of(c(M_tumor_col, M_ex_col)))))) ~ "group 14",

      (group == "group X") &
      ((identical_all(c_across(all_of(c(m_tumor_col, m_in_col))))) | 
      (identical_all(c_across(all_of(c(M_tumor_col, M_in_col)))))) ~ "group 15",

      (group == "group X") &
      ((identical_all(c_across(all_of(c(m_tumor2_col, m_ex_col))))) |
      (identical_all(c_across(all_of(c(M_tumor2_col, M_ex_col)))))) ~ "group 16",

      (group == "group X") &
      ((identical_all(c_across(all_of(c(m_tumor2_col, m_in_col))))) | 
      (identical_all(c_across(all_of(c(M_tumor2_col, M_in_col)))))) ~ "group 17",

      (group == "group X") &
      ((identical_all(c_across(all_of(c(m_ex_col, m_in_col))))) |
      (identical_all(c_across(all_of(c(M_ex_col, M_in_col)))))) ~ "group 18",

      (group == "group X") ~ "group 19", 
      
      TRUE ~ group
    )) %>% 
    mutate(identical = case_when(
      # Determine if the values are identical in both M and m columns
      group %in% c("group 1", "group 2", "group 3", "group 4", "group 5", "group 6", "group 7", "group 8", "group 9", "group 10", "group 11") ~ "M,m",
      
      (group_2 == "group 12") && (identical_all(c_across(all_of(c(m_tumor_col, m_tumor2_col, m_ex_col, m_in_col))))) ~ "m",
      (group_2 == "group 12") && (identical_all(c_across(all_of(c(M_tumor_col, M_tumor2_col, M_ex_col, M_in_col))))) ~ "M",

      (group_2 == "group 13") && (identical_all(c_across(all_of(c(m_tumor_col, m_tumor2_col)))))  ~ "m",
      (group_2 == "group 13") && (identical_all(c_across(all_of(c(M_tumor_col, M_tumor2_col))))) ~ "M",

      (group_2 == "group 14") && (identical_all(c_across(all_of(c(m_tumor_col, m_ex_col)))))  ~ "m",
      (group_2 == "group 14") && (identical_all(c_across(all_of(c(M_tumor_col, M_ex_col))))) ~ "M",

      (group_2 == "group 15") && (identical_all(c_across(all_of(c(m_tumor_col, m_in_col))))) ~ "m",
      (group_2 == "group 15") && (identical_all(c_across(all_of(c(M_tumor_col, M_in_col))))) ~ "M",

      (group_2 == "group 16") && (identical_all(c_across(all_of(c(m_tumor2_col, m_ex_col))))) ~ "m",
      (group_2 == "group 16") && (identical_all(c_across(all_of(c(M_tumor2_col, M_ex_col))))) ~ "M",

      (group_2 == "group 17") && (identical_all(c_across(all_of(c(m_tumor2_col, m_in_col))))) ~ "m",
      (group_2 == "group 17") && (identical_all(c_across(all_of(c(M_tumor2_col, M_in_col))))) ~ "M",

      (group_2 == "group 18") && (identical_all(c_across(all_of(c(m_ex_col, m_in_col))))) ~ "m",
      (group_2 == "group 18") && (identical_all(c_across(all_of(c(M_ex_col, M_in_col))))) ~ "M",
...
drivers_cn = drivers_cn %>% left_join(classified_cnv[,c(1,10)], by = "cnv_id") %>% 
...
drivers_cn = drivers_cn %>% mutate(total_cp_tumor = .data[[M_tumor_col]] + .data[[m_tumor_col]],
                                  total_cp_tumor2 = .data[[M_tumor2_col]] + .data[[m_tumor2_col]],
...
Deleted_drivers <- drivers_cn %>% 
  filter((total_cp_tumor == 0) | (total_cp_tumor2 == 0) | (total_cp_ex == 0) | (total_cp_in == 0) )  %>% as.data.frame()

#Find amplification in any Allele in any sample of Totoki driver genes

Amp_drivers <- drivers_cn %>% 
  filter((total_cp_tumor > 3) | (total_cp_tumor2 > 3) | (total_cp_ex > 3) | (total_cp_in > 3) ) %>% select(driver_gene,total_cp_tumor,total_cp_tumor2, total_cp_ex, total_cp_in)
...
pred_muts %>% filter(Gene_Annovar == "RNF43")
