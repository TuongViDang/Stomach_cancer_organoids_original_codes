...
#SNV
Conifer_output = "/group/poetsch_projects/poetsch_sc/Conifer/OO100/output/Clustering/OO100.SCoutput.CLEAN.tsv"
...
#CNV
Ascat_dir="/group/poetsch_projects/poetsch_sc/AscatNGS/OO100"
tumor = read.delim(file.path(Ascat_dir,"/OO100_tumor/result/OO100_OO100_tumor.copynumber.caveman.txt"), header=F)
...
ex = read.delim(file.path(Ascat_dir,"/OO100_ex/result/OO100_OO100_ex.copynumber.caveman.txt"), header=F)
...
inv = read.delim(file.path(Ascat_dir,"/OO100_in/result/OO100_OO100_in.copynumber.caveman.txt"), header=F)
...
write.table(disjoint_df_bed, "/group/poetsch_projects/poetsch_sc/ClassifyCNV/OO100/OO100_disjoint/OO100_disjoint.bed", quote=FALSE, row.names = F)


#Run bash 
conda activate ClassifyCNV
bash update_clingen.sh
main_dir="/group/poetsch_projects/poetsch_sc/ClassifyCNV/OO100/OO100_disjoint/"
mkdir $main_dir/output
python3 /group/poetsch_projects/poetsch_sc/ClassifyCNV/ClassifyCNV.py  --infile $main_dir/OO100_disjoint.bed --GenomeBuild hg38 --outdir $main_dir/output

cd $output_dir

awk '$6 == "Pathogenic"' Scoresheet.txt  > OO100_disjoint.Result.Pathogenic.txt

#### R ######
#reload result of ClassifyCNV
annotated_disjoint_df = read.delim("/group/poetsch_projects/poetsch_sc/ClassifyCNV/OO100/OO100_disjoint/output/Scoresheet.txt", header=T)
...
#Y data
#Create_Y_data <- function(i, disjoint_df) {
#    chr = paste0("chr",snv_df_unique$CHR[i])
#    pos = snv_df_unique$POS[i]
    
#    result <- data.frame(mut = snv_df_unique$mutation_id[i], cnv = "non-cna")  # Default result

#    for (j in (1:nrow(disjoint_df))) {
#        if (chr == disjoint_df$seqnames[j]) { 
#            if ((pos >= disjoint_df$start[j]) & (pos <= disjoint_df$end[j])) {
#                result = data.frame(mut = snv_df_unique$mutation_id[i], cnv = disjoint_df$cnv_id[j])
                # break
           # }
        # }
    # }    
...
# snv_df_unique <- snv_df %>% distinct(mutation_id, .keep_all = TRUE)

# Y_data = purrr::map_dfr(1:nrow(snv_df_unique),~Create_Y_data(.x,disjoint_df=disjoint_df))
# Y_data = as.matrix(table(Y_data$mut,Y_data$cnv))
# Y_data = as.matrix(Y_data[,c(123,1:122)])


#Filter the disjoint_df
# kept = intersect(filtered_disjoint_df$cnv_id, colnames(Y_data))
# disjoint_df <- disjoint_df %>% filter(cnv_id %in% kept)
...
tumor_new = tumor_new %>% mutate( cnv_id = paste0(sub("chr","",chr) ,":",  start  , ":",  end ), sample = "OO100_tumor")
tumor_new_M = tumor_new %>% select(cnv_id,sample, M)
tumor_new_m = tumor_new %>% select(cnv_id,sample, m)

ex_new = purrr::map_dfr(1:nrow(disjoint_df),~CN_for_disjoint_seq(.x,df= ex))
ex_new = ex_new %>% mutate( cnv_id = paste0(sub("chr","",chr) ,":",  start  , ":",  end ), sample = "OO100_ex")
...
inv_new = inv_new %>% mutate( cnv_id = paste0(sub("chr","",chr) ,":",  start  , ":",  end ), sample = "OO100_in")
...
Find_driver_cn = function(driver_gene, combined_data){
  ...
  cnv = cnv %>% select(driver_gene,cnv_id, M_value_OO100_tumor ,M_value_OO100_ex, M_value_OO100_in,  m_value_OO100_tumor, m_value_OO100_ex, m_value_OO100_in)
}
#COPY NUMBER of all TOTOKI gene
drivers_cn = purrr::map_dfr(driver_genes,~Find_driver_cn(.x,combined_data = combined_data))


#CANOPY - does not work
#projectname = 'OO100'
#R =  snv_AD
#X = snv_DP
#WM = M_data
#Wm = m_data
#Y = Y_data
#C = diag(1, nrow(M_data), nrow(M_data))
...
#png("/group/poetsch_projects/poetsch_sc/CANOPY/OO100.likelihood.png")
...
pred_muts %>% filter(Gene_Annovar == "MDM2")